...
export class Computer {
  ...
  private async playOneCard(id: number) {
    if (id === -1) return;
  ...
  async randomPlayAttck() {
    ...
    const cardsSpecial = this.self.handCards.filter(
    ...
    let play3attack = false;
    {
      ...
      for (const card of cardsSpecial) {
        if (card.name === "防火墙升级") {
          const cardsDefense = this.self.handCards.filter(
    ...
    (() => {
      if (id1 !== -1) return;
      //判断手上是否同时有web攻击和提权攻击
      const web = cards.find(
        (item) => item.tag2 === "WEB_ATTACK" && item.name !== "提权攻击"
      );
    ...
    (() => {
      ...
      const c2 = cards.find(
        (item) => item.name === "弱口令攻击" && item.id !== c1.id
      );
    ...
    (() => {
      ...
      //判断手上是否有社工和恶意广告
      const c1 = cards.find(
        (item) => item.tag2 === "SOCIAL_ENGINEERING" && item.name !== "恶意广告"
      );
